...
$(document).ready(function () {
    AjaxCall('/ProfileDevHead/CheckSupervisorProfile', 'POST').done(function (response) {
        if (response == true) {
            Swal.fire({
                icon: 'warning',
...
btnCancel.addEventListener('click', () => {
    window.location.href = '/ProfileDevHead/ViewProfileDevHead'
})
...
$('.btnSave').on('click', function () {
    ...
    AjaxCall('/ProfileDevHead/EditProfileSupervisor', JSON.stringify(supervisor), 'POST').done(function (respone) {
        if (respone == 1)
            Swal.fire({
                icon: 'success',
                ...
                window.location = "/ProfileDevHead/ViewProfileDevHead";
            });
        else {
            Swal.fire({
                icon: 'error',
...
$(document).ready(function () {
    ...
    $('#emailContactInfo').blur(function () {
        if (!validFeEmail.test($('#emailContactInfo').val().trim())) {
