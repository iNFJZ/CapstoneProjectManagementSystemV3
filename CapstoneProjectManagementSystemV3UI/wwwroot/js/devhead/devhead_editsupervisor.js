...
const txt_UserID = document.querySelector('#UserIdInfo');
//$(document).ready(function () {
//    AjaxCall('/EditSupervisor/CheckSupervisorProfile', 'POST').done(function (response) {
//        if (response == true) {
//            Swal.fire({
//                icon: 'warning',
...
btnCancel.addEventListener('click', () => {
    window.location.href = '/ViewSupervisorForDevHead/DetailSupervisorWithRoleDevHead/' + txt_UserID.value
})
...
$('.btnSave').on('click', function () {
    ...
    user.UserID = txt_UserID.value;

    //object student
    var supervisor = {};

    supervisor.PhoneNumber = $('#phoneContactInfo').val();
    /*supervisor.MaxGroup = $('#maxGroupBasicInfo').val();*/
    supervisor.SelfDescription = $('.formBasicInfo__bio textarea').val().trim();
    ...
    AjaxCall('/EditSupervisor/EditProfileSupervisor', JSON.stringify(supervisor), 'POST').done(function (respone) {
        if (respone == 1)
            Swal.fire({
                icon: 'success',
                ...
                window.location = "/ViewSupervisorForDevHead/DetailSupervisorWithRoleDevHead/" + txt_UserID.value;
            });
        else {
            Swal.fire({
                icon: 'error',
...
$(document).ready(function () {
    ...
    //FE email
    var validFeEmail = /^[^\s@]+@fe\.edu\.vn$/;
    $('#emailContactInfo').blur(function () {
        if (!validFeEmail.test($('#emailContactInfo').val())) {
            $('#erroremailFeContactInfo').html('Email must have a format example@fe.edu.vn'); 
        } else {
            $('#erroremailFeContactInfo').html('');
        }
    });
    ...
    $('#personalEmailContactInfo').blur(function () {
        if (!validEmail.test($('#personalEmailContactInfo').val())) {
