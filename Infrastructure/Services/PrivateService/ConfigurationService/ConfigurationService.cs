...
namespace Infrastructure.Services.PrivateService.ConfigurationService
{
    public class ConfigurationService : IConfigurationService
    {
        ...
        public async Task<ApiResult<List<WithDto>>> GetWithProfessionBySpecialityId(int specialtyId)
        {
            var specialty = await _specialtyRepository.GetById(s => s.SpecialtyId == specialtyId);
            if (specialty == null)
            {
                return new ApiErrorResult<List<WithDto>>("Specialty not found");
            }
            ...
            //    }
            //}).ToList();
            var result = new List<WithDto>();
            foreach (var with in withs)
            {
                result.Add(new WithDto()
                {
                    Profession = new ProfessionDto()
                    {
                        ProfessionID = with.ProfessionId.Value,
                    },
                    Specialty = new SpecialtyDto()
                    {
                        SpecialtyID = with.SpecialtyId.Value,
                    }
                });
            };
            // Step 5: Return the list wrapped in ApiResult
            return new ApiSuccessResult<List<WithDto>>(result);
        }

        //
        public async Task<ApiResult<List<WithDto>>> GetWithsBySpecialtyID(int specialtyID)
        {
        ...
        public async Task<ApiResult<bool>> Insert(int Specialty, List<WithDto> Withs, int Profession)
        {
